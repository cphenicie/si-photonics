...
plt.title('Optical spectra for the YBranch test structures', fontsize = FONTSIZE)
...
plt.xlabel('Number of YBranches', fontsize = FONTSIZE)
plt.ylabel('Insertion Loss (dB)', fontsize = FONTSIZE)
plt.title('Cut-back method, YBranch, insertion loss, at 1550 nm', fontsize = FONTSIZE)
plt.legend()
...
plt.annotate('Fit results, with 95% confidence intervals:', xy=(6, -28))
plt.annotate('slope is ='+'%.3f' %A[1] + ' +/- ' + '%.3f' %SlopeError95CI + ' dB/YBranch', xy=(6, -29))
plt.annotate('intercept is = '+'%.3f' %A[0] + ' +/- ' + '%.3f' %InterceptError95CI + 'dB', xy=(6, -30))
plt.show()
print ('Cut-back method, YBranch (TM) insertion loss, at', lam0*1e9, 'nm is =', -A[1], 'dB/YBranch')
...
plt.legend(loc = 2)
plt.title('Cut-back method, YBranch, insertion loss, wavelength dependance')
