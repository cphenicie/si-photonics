...
dL = 300;  % 297.4; % um
data_name = 'data\christopherphenicie_MZI300um.mat';
...
% range!!
lambda_min = 1.514e-6;  % 1.521e-6;
lambda_max = 1.578e-6;  % 1.569e-6;
% lambda0 = 1.545;
...
plot(lambda_fit*1e6 + 0.0032, fit_T_MZI(x0, lambda_fit*1e6), 'DisplayName', 'Simulation');
...
%title(sprintf('Measured FSR = %.2f nm', fsr*1e3))
title(sprintf('Measured FSR = %.2f nm -> n_g = %.5f at 1550', ...
    fsr*1e3, 1.55^2 / (fsr * dL)))
...
% 
% nx_init = [n1, n2, n3]
nx_init = [2.4569, -1.13, -0.063];
% nx_init = simulated_n;

alpha_init = 4.6e-3;  % propagation loss [micron^-1]
device_loss = 1.28;
...
[xfit,resnorm,residual,exitflag,output,lambda,jacobian] = ...
    lsqcurvefit(fit_T_MZI, x0, lambda_fit*1e6, amplitude_fit, [], [], options);
ci = nlparci(xfit,residual,'jacobian',jacobian);
xerr = (ci(:,1) - ci(:,2)) / 4;
xfit
xerr'
save('MZI300um_fit.mat', 'xfit', 'xerr', 'fsr', 'fsr_err', 'dL')
