...
% Device is labelled "MI4", it's on the bottom of the layout
dL = 2*(2159.66 - 11.69) + 87.81 - 82.63; % 2*(2297.40 - 11.69) + 87.81 - 82.63;  % 297.4; % um
% dL = 2*2200;  % 297.4; % um
% dL = 2*2150;  % 297.4; % um
data_name = 'data\christopherphenicie_MIA.mat';

loopback_name = 'data\christopherphenicie_GCCal.mat';
...
fit_T_MZI_anylength = @(X, lambda) ...
            T_MZI_anylength(lambda0, X, lambda);
% Background correction means no losses, X(4) (=alpha) = 0 and X(5) = 0
x0 = [simulated_n, 0, 0, dL] ;
...
x_data = lambda_fit*1e6;
y_data = smoothdata(amplitude_fit, 'gaussian', 3);

plot(x_data, y_data)
[pks, locs] = findpeaks(y_data);
good_locs_idx = find(1.54 < x_data(locs) & x_data(locs) < 1.56);
max_wl = x_data(locs(good_locs_idx));
plot(max_wl, pks, 'o')

lambda = mean(lambda_fit);
fsr = abs(mean(diff(max_wl)));
fsr_err = std(diff(max_wl));
% ng = lambda^2 / (fsr * delta_L)
% ng = lambda^2 / (fsr * delta_L_list(i))

xlabel('Wavelength [nm]')
ylabel('Transmission [I_o / I_i]')
title(sprintf('Measured FSR = %.2f nm -> deltaL = %.2f um at ng = 4.21 and 1550nm',...
    fsr*1e3, 1.55^2/(4.21*fsr)))
...
nx_init = [2.4698, -1.13, -0.1]; % [2.46, -1.1385, -0.0646]
% nx_init = simulated_n;

alpha_init = 0.01e-3;  % propagation loss [micron^-1]
device_loss = 0.001;
...
lb = [2.43, -inf, -1.5, -1, -1];
ub = [2.48, inf, 0, inf, inf];

[xfit,resnorm,residual,exitflag,output,lambda,jacobian] = ...
    lsqcurvefit(fit_T_MZI, x0, lambda_fit*1e6, amplitude_fit, lb, ub, options);
ci = nlparci(xfit,residual,'jacobian',jacobian);
xerr = (ci(:,1) - ci(:,2)) / 4;

xfit'
xerr
save('MIA_fit.mat', 'xfit', 'xerr', 'fsr', 'fsr_err', 'dL')

% [xfit,resnorm] = lsqcurvefit(fit_T_MZI_anylength, x0, lambda_fit*1e6, amplitude_fit,... 
%     lb, ub, options);
% xfit(1:5)
% xfit(6)
...
% MZI transfer function
function t = T_MZI_anylength(lambda0, X, lambda)
    % lambda0: Center wavelength for the waveguide compact model (in um)
    % dL: The MZI length imbalance (in um)
    % X(1:3): the n1, n2, n3 parameters for the waveguide compact model
    % X(4): the waveguide attenuation coefficient ("alpha") in 1/um
    % X(5): the wavelength-independent loss in the device (in dB).
    % X(6): The length imbalance of the interferometer
    % lambda: List of wavelengths (in um)

    b = beta(lambda0, X(1:3), X(4), lambda);
    t = (10*log10( 0.25 * abs(1+exp(-1i*b*X(6))).^2) + X(5) );
end
